@extends('layouts.app', ['page' => __('Edit Property'), 'pageSlug' => 'properties'])
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="title">{{ __('Edit Property') }}</h5>
                </div>
                <form method="post" action="{{ route('properties.update', ['property' => $property]) }}" autocomplete="off">
                    <div class="card-body">
                        ...
                        <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                            <label>{{ __('Name') }}</label>
                            <input type="text" name="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name') }}" value="{{ old('name', $property->name) }}">
                            @include('alerts.feedback', ['field' => 'name'])
                        </div>

                        <div class="form-group{{ $errors->has('city') ? ' has-danger' : '' }}">
                            <label>{{ __('City') }}</label>
                            <input type="text" name="city" class="form-control{{ $errors->has('city') ? ' is-invalid' : '' }}" placeholder="{{ __('City') }}" value="{{ old('city', $property->city) }}">
                            @include('alerts.feedback', ['field' => 'city'])
                        </div>

                        <div class="form-group{{ $errors->has('address') ? ' has-danger' : '' }}">
                            <label>{{ __('Address') }}</label>
                            <input type="text" name="address" class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" placeholder="{{ __('Address') }}" value="{{ old('address', $property->address) }}">
                            @include('alerts.feedback', ['field' => 'address'])
                        </div>

                        <div class="form-group{{ $errors->has('price') ? ' has-danger' : '' }}">
                            <label>{{ __('Price') }}</label>
                            <input type="text" name="price" class="form-control{{ $errors->has('price') ? ' is-invalid' : '' }}" placeholder="{{ __('Price') }}" value="{{ old('price', $property->price) }}">
                            @include('alerts.feedback', ['field' => 'price'])
                        </div>

                        <div class="form-group{{ $errors->has('surface') ? ' has-danger' : '' }}">
                            <label>{{ __('Surface') }}</label>
                            <input type="number" name="surface" min="1" class="form-control{{ $errors->has('surface') ? ' is-invalid' : '' }}" placeholder="{{ __('Surface') }}" value="{{ old('surface', $property->surface) }}">
                        ...
                        <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                            ...
                            <textarea type="text" name="description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" placeholder="{{ __('Description') }}">{{ old('description', $property->description) }}</textarea>
